...
def encontrar_jurado_estricto(matriz_puntuacion: list, indice: int) -> list:
    ...
    Returns:
        list: Una lista conteniendo dos elementos. Una lista con los indices de jurados mas estrictos,
...
def encontrar_top_3(matriz_puntuacion: list) -> list[list]:
    ...
    Returns:
        list[list]: Una lista de listas. Contiene listas con dos elementos.
    ...
    if num_participantes > 0:
        num_jurados = len(matriz_puntuacion[0])
    ...
    while i < num_participantes:
        ...
        while j < num_jurados:
            ...
            j += 1
        promedio = calcular_promedio(suma, num_jurados)
        promedios += [promedio]
        indices += [i]
        i += 1
...
def desempatar(desempate: str, cantidad_ganadores: int, ganador: int = None) -> int:
    """Comprueba si hay que desempatar. Si hay que hacerlo, genera un numero al azar. Si no, avisa que no hay que desempatar. 
    Si no se comprobaron los ganadores, pide que el usuario los compruebe.

    Args:
        desempate (str): "Si" si hay que desempatar. "No" si no. Cualquier otro valor si no se comprobaron los ganadores.
        cantidad_ganadores (int): Cantidad de ganadores, usada para limitar el rango de numeros a generar.
        ganador (int, optional): El numero que se genera para decidir al ganador. Por defecto es None para realizar
        comprobaciones.

    Returns:
        int: El numero generado para decidir al ganador.
    """
    if desempate == "No":
...
def mostrar_desempate(array_participantes: list, matriz_puntuacion: list, desempate: str, ganador: int = None) -> int:
    """Comprueba si hay un numero asignado a ganador. Si lo hay, muestra quien es el ganador. 
    Si no, intenta generarlo, si no puede devuelve None. 
    Si ya se decidio un ganador, informa que ya se decidio previamente, y muestra su info.

    Args:
        array_participantes (list): Lista con los nombres de los participantes.
        matriz_puntuacion (_type_): Matriz con todas las puntuaciones dadas a los participantes por los jueces.
        desempate (str): "Si" si se debe desempatar, "No" si no. Se utiliza para determinar si generar un ganador. 
        ganador (int, optional): El numero que se genera para decidir al ganador. Si es None, se intenta generar uno nuevo.

    Returns:
        int: El numero generado para decidir al ganador. Se retorna para realizar comprobaciones mas adelante.
    """
